...
namespace Ophim\ThemeBptv\Controllers;
...
class ThemeBptvController
{
    public function index(Request $request)
    {
        if ($request['search'] || $request['filter']) {
            ...
            return view('themes::themebptv.catalog', [
                'data' => $data,
                'search' => $request['search'],
                'section_name' => "Tìm kiếm phim: $request->search"
            ]);
        }
        return view('themes::themebptv.index', [
    ...
    public function getMovieOverview(Request $request)
    {
        ...
        $movie->increment('view_total', 2);
        $movie->increment('view_day', 2);
        $movie->increment('view_week', 2);
        $movie->increment('view_month', 2);
    ...
    public function lichchieuphim()
    {
        return view('themes::themebptv.lichchieuphim');
    }
}
