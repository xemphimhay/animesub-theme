@extends('themes::themebptv.layout_2')
...
@php
    ...
    $next_url = '';
    $prev_url = '';
    foreach ($currentMovie->episodes->where('server', $episode->server) as $data) {
        if ($data->slug == $episode->slug) {
            $current_url = $data->getUrl();
        }
        $urls[] = $data->getUrl();
    }
    foreach ($urls as $key => $url) {
        if ($current_url == $url) {
            $next_url = ($key + 1) >= count($urls) ? $current_url : $urls[$key + 1];
            $prev_url = ($key - 1) <= 0 ? $current_url : $urls[$key - 1];
        }
    }
@endphp
    ...
    @endif-->
        ...
        <div id="watch-block">
            ...
            <div class="MovieTabNav ControlPlayer">
                <a class="Lnk AAIco-skip_next" id="btn-nextepisode" href="{{ $next_url }}">Tập tiếp </a>
                ...
                <div class="Lnk fa-ban" id="btn-toggle-download" title="Tải về">Tắt Ads</div>
            </div>
        </div>
                                ...
                                <div class="Image">
                                    ...
                                    <div class="anime-extras">
                                        <div class="anime-avg-user-rating"
                                            title="{{ $movie->getRatingStar() }} trong số {{ $movie->rating_count }} đánh giá"
                                            data-action="click->anime-card#showLibraryEditor"><i
                                                class="fa fa-star"></i>{{ $movie->getRatingStar() }}
                                        </div>
                                    </div>
                                </div>
